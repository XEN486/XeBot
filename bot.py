import discord, properties, os, asyncio
...
async def on_ready():
    ...
    for extension in extensions:
        ...
        await bot.load_extension(f'Extensions.{extension}.{extension}')
    extensions.remove('Jishaku') # hide it from view
    print(f'Logged in as user \'{bot.user.name}#{bot.user.discriminator}\' with id \'{bot.user.id}\'')
...
async def unload(ctx, extension):
    await bot.unload_extension(f'Extensions.{extension}.{extension}')
...
async def reload(ctx, extension):
    if ctx.author.id not in properties.OWNERS:
        ...
        await bot.reload_extension(f'Extensions.{extension}.{extension}')
